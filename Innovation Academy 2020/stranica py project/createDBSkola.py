...
prezimebalabovic = '''
                Select IDNastavnika
                From Nastavnici
                Where Prezime="Balabovic"'''

trgod='''
                Select IDGodine
                From Skolskegodine
                Where oznakaGod="2020"'''

prb='''
                Select IDUcenika,Ime,Prezime,Datumrodenja
                From Ucenici
                Where Ime LIKE "B%"
...
prc='''
                Select Ime,Prezime
                From Ucenici
                Where Prezime LIKE "%c"'''

prsij='''
                Select Ime,Prezime,Datumrodenja
                From Ucenici
                Where Datumrodenja LIKE "%-01-%"
                ORDER BY Datumrodenja'''
raz='''
                Select oznakaRaz
                From Razredi
                ORDER BY oznakaRaz'''
...
def main():
    ...
    # for row in data:
    #     print(row)
    lista=c.execute(prezimebalabovic)
    for l in lista:
        print(l)
    lista1=c.execute(trgod)
    for l in lista1:
        print(l)
    lista2=c.execute(prb)
    for l in lista2:
        print(l)
    lista3=c.execute(prc)
    for l in lista3:
        print(l)
    lista4=c.execute(prsij)
    for l in lista4:
        print(l)
    lista5=c.execute(raz)
    for l in lista5:
        print(l)
